...
import { rumorBeliefChance, applyRelationshipImpact } from './relationships.js';

export function createRumor(world, { claim, sourceAgentId, targetAgentIds = [], truthLevel = 50, emotionalTone = 'suspicion', spreadRate = 30, originEventId }) {
  const rumor = {
    id: world.nextId('rumor'), claim, sourceAgentId, targetAgentIds,
    truthLevel: clamp(truthLevel, 0, 100), emotionalTone, spreadRate: clamp(spreadRate, 0, 100),
    knownByAgentIds: sourceAgentId ? [sourceAgentId] : [], originEventId, createdAtTick: world.tick,
    distortionLevel: 0, active: true
  };
  world.rumors[rumor.id] = rumor;
  return rumor;
}

export function spreadRumorTo(world, rumorId, listenerAgentId, sourceAgentId) {
  ...
  return world.addEvent({
    type: 'rumor_spread', locationId: world.agents[listenerAgentId].locationId,
    actorIds: unique([sourceAgentId, listenerAgentId].filter(Boolean)),
    description: `${world.agents[listenerAgentId].name} heard rumor: ${rumor.claim}`,
    public: false, visibleToAgentIds: [listenerAgentId, sourceAgentId].filter(Boolean),
...
export function propagateRumors(world) {
  const events = [];
...
export function counterRumor(world, rumorId, { counterClaim, evidenceStrength = 0, actorId = 'player' }) {
  const rumor = world.rumors[rumorId];
  ...
  return world.addEvent({
    type: 'counter_rumor', locationId: world.agents[actorId]?.locationId ?? 'market', actorIds: [actorId],
...
export function traceRumor(world, rumorId, { actorId = 'player', evidenceStrength = 0 } = {}) {
  const rumor = world.rumors[rumorId];
  if (!rumor) throw new Error(`Rumor not found: ${rumorId}`);
  ...
  return world.addEvent({
    type: 'rumor_traced', locationId: world.agents[actorId]?.locationId ?? 'market', actorIds: [actorId],
    description, public: false, visibleToAgentIds: [actorId], causes: [rumor.originEventId].filter(Boolean),
    consequences: [{ type: 'rumor_trace', rumorId, sourceRevealed: canRevealSource }],
    importance: canRevealSource ? 4 : 3, payload: { rumorId, sourceRevealed: canRevealSource }
  });
}
